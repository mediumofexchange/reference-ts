...
| `model/pool-v3-ergo-profile.ts`, `experiments/ergo-range/replay-venue.mjs` | [Candidate Ergo venue profile](ERGO_VENUE_PROFILE.md): identity over genesis, depth and four exact locations; attribution by tree and `R4`/`R5` shape; run reassembly; transaction-then-output ordinals; a §13 verifier by exhaustion over root-checked blocks behind a linked header chain, checked on synthetic chains through Fleet and sigma-rust and on the three fixture roots. The bounded adapter replays real-proof imports/payments/burns and fresh readers from raw transaction bytes under separate synthetic headers. | Selection by decision with real-chain cost evidence, an authenticated header source, a contained decoder, publication on a node and runtime adoption in place of the v2 materialized view; the kind-4 bound exceeds one Ergo transaction. |
