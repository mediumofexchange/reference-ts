...
| `model/pool-fault.ts` | Unselected intrinsic exclusion and competing clocks, including the proposed snapshot clock and segment continuity switches; independent evidence snapshots and original-prefix revalidation. [Fault recovery](POOL_FAULT_RECOVERY.md) defines the recommendations, dependencies and actual-v2 limits; the companion's `pool-fault.md` is the specification proposal awaiting selection. Do not consolidate it into the normative model yet. |
