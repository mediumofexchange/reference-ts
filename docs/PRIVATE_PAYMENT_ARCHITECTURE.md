...
| `ergo.ts`, `ergo-headers.ts`, `ergo-profile.ts`, `ergo-supplier.ts`, `record-range.ts` | The [selected Ergo venue profile](ERGO_VENUE_PROFILE.md#runtime-venue) as the runtime's read-only `Venue`: headers verified from the anchor's context and sections by root from untrusted suppliers under per-supplier budgets, §13.3 reads by exhaustion and §13 answers from one snapshot published atomically, which reads use while a sync runs; a missing section stops the clock, a reorganization past the depth fails the view. | Publishing (the operator's wallet), persistence across restarts, side-branch pruning. Not exported from the root barrel. |
