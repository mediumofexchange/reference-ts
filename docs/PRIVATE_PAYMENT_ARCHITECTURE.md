...
|---|---|---|
| `scripts/pool/v3/local-{replay,worker,check}.mjs`, `scripts/pool/v3/fixture-venue.mjs` | Real-proof issue/spend/burn replay and candidate note paths; checked candidate configuration and signed terms-derived issuer key, with a fresh seedless audit process. §13 record-range reads against a harness-owned fixture venue establish the replacement chain, the checkpoint's record prefix, currency, its operator's force and revocation absence for that fixture only, and every carrying checkpoint of the segment is classified from its own trail with last-valid-prefix continuity; under a declared clause the no-commitment clock, the silence boundary and lapse by silence are read from the same walk (C2b.6.1, C2b.4.1). Adoption, a selected venue profile, imports and successor segments remain unresolved. | V3 runtime takes over these checks with approved configuration, recovery/import replay, a venue profile behind the range answers and finality. |
...
| `model/pool-v3-package.ts`, `scripts/pool/v3/local-replay.mjs` | Canonical source-neutral evidence inventory and existing directory preimage; fresh local readers use bounded package bytes and the same replay engine. No complete-certificate verdict. | V3 runtime takes over transport and hostile cases with complete dependency replay and an authenticated venue evidence profile. |
| `model/pool-v3-range.ts` | Source-neutral record-range request/answer frame, held commitments by sequence with reader-established priors, replacement identities and C2.5's chain walk, first-entry revocations and cross-backing publication order; answers are the reader's own verifier output. No venue profile or verdict. | V3 runtime takes over the codec and reader rules behind a selected venue profile with authenticated chain evidence; no second production reader. |
| `model/pool-v3-ergo-profile.ts`, `experiments/ergo-range/profile-check.mjs` | [Candidate Ergo venue profile](ERGO_VENUE_PROFILE.md): identity over genesis, depth and four exact locations; attribution by tree and `R4`/`R5` shape; run reassembly; transaction-then-output ordinals; a §13 verifier by exhaustion over root-checked blocks behind a linked header chain, checked on synthetic chains through Fleet and sigma-rust and on the three fixture roots. | Selection by decision with real-chain cost evidence, an authenticated header source, a contained decoder, publication on a node and runtime adoption in place of the v2 materialized view; the kind-4 bound exceeds one Ergo transaction. |
