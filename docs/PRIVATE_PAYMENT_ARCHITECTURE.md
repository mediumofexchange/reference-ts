...
| `model/pool-v3-package.ts`, `scripts/pool/v3/local-replay.mjs` | Canonical source-neutral evidence inventory and existing directory preimage; fresh local readers use bounded package bytes and the same replay engine. No complete-certificate or range verdict. | V3 runtime takes over transport and hostile cases with complete dependency replay and an authenticated venue evidence profile. |
