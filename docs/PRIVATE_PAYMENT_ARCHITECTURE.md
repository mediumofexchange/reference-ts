...
| `model/pool-v3-ergo-profile.ts`, `model/pool-v3-ergo-headers.ts`, `experiments/ergo-range/replay-venue.mjs` | [Selected Ergo venue profile](ERGO_VENUE_PROFILE.md) ([venue-ergo.md](https://github.com/mediumofexchange/money-from-first-principles/blob/13e5b66/venue-ergo.md)): identity over a pinned anchor header, depth and four exact locations; index 0 the anchor's child; attribution by tree and `R4`/`R5` shape; run reassembly; transaction-then-output ordinals; a §13 verifier by exhaustion over root-checked blocks behind a linked header chain from the anchor's child, checked on synthetic chains through Fleet and sigma-rust and on the four fixture roots. The bounded adapter replays every real-proof local replay group and fresh readers from raw transaction bytes under separate synthetic headers. The reader's header store verifies mainnet headers' work and EIP-37 difficulty from the anchor, so header suppliers are untrusted ([header source](ERGO_VENUE_PROFILE.md#header-source)). | Runtime adoption in place of the v2 materialized view, with a header supplier policy and default depth 10; the framer's grammar checked against the deployment's publications. |
