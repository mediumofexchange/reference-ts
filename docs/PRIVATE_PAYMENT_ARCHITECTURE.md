...
| `pool/v3/` (candidate) | pool-v3's codecs, promoted from `model/` with their tests: records and publications (§§5–6); history, evidence, snapshot and receipt frames (§7); segment headers (§8); fault evidence (§9); served trails (§10); configuration and signed root terms (§11); evidence packages (§12). With them the compressed spent root (pool-spent C1.2.8–9) and receiver-prepared outputs, capsules and seed scanning (pool-delivery C4.2–C4.4, C4.6–C4.7). The state machine (`state.ts`: one §5 record applied to a segment's replayed state, in replay and adoption modes, every refusal named; single-backing recovery state in `recovery.ts`) and the single-segment reader (`reader.ts`: §13 record reads through a `RecordVenue`, a checkpoint's trail replayed through the state machine with resumption from the last valid checkpoint, and the original segment's carrying checkpoints classified with the C2b.6.1 clock and C2b.4.1 lapse; `served-trail.ts`, `refusals.ts`). `test/neutral-core.test.ts` pins that v3 imports only the neutral core; the v3 scripts read it from `dist/`, and the replay harness layers imports, scopes, receipts, recovery force and non-service counts over it. | No admission mode, journal, prover or candidate guard (M2); publication force, imports and scopes stay in the harness until slices 3 and 7; no adopted configuration. Not exported from the root barrel. |
