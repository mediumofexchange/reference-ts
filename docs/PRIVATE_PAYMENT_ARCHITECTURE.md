...
| `ergo.ts`, `ergo-headers.ts`, `ergo-profile.ts`, `ergo-supplier.ts`, `ergo-publisher.ts`, `record-range.ts` | The [selected Ergo venue profile](ERGO_VENUE_PROFILE.md#runtime-venue) as the runtime's `Venue` and `RecordVenue`, the one Ergo reader, publishing kind 1–3 records through an optional funding-key wallet that builds and signs its own transactions: headers verified from the anchor's context and sections by root from untrusted suppliers under per-supplier budgets, §13.3 reads by exhaustion and §13 answers from one snapshot published atomically, which reads use while a sync runs; a missing section stops the clock, a reorganization past the depth fails the view. | Persistence across restarts (the publisher's memory included), side-branch pruning, kind-4 runs. Not exported from the root barrel. |
| `pool/v3/` (candidate) | pool-v3's codecs, promoted from `model/` with their tests: records and publications (§§5–6); history, evidence, snapshot and receipt frames (§7); segment headers (§8); fault evidence (§9); served trails (§10); configuration and signed root terms (§11); evidence packages (§12). With them the compressed spent root (pool-spent C1.2.8–9) and receiver-prepared outputs, capsules and seed scanning (pool-delivery C4.2–C4.4, C4.6–C4.7). The state machine (`state.ts`: one §5 record applied to a segment's replayed state, in admission, replay and adoption modes, every refusal named; single-backing recovery state in `recovery.ts`) and the single-segment reader (`reader.ts`: §13 record reads through a `RecordVenue`, a checkpoint's trail replayed through the state machine with resumption from the last valid checkpoint, and the original segment's carrying checkpoints classified with the C2b.6.1 clock and C2b.4.1 lapse; `served-trail.ts`, `refusals.ts`). The operator side: `witness.ts` builds issue, spend and burn circuit inputs and public inputs from notes and prepared outputs; `prover.ts` proves them with `noir_js` and bb.js (optional peers) after deriving all six identities and requiring the configuration's; `store.ts` is the operator journal (SQLite command log fenced to one owner, genesis opening of one backing, kinds 1–3 admitted in admission mode with original receipts, checkpoint commitments, an outbox publishing through a `RecordPublisher`, served §12 packages); `guard.ts` refuses every venue whose identity does not recompute from a reference preimage the caller holds. `test/neutral-core.test.ts` pins that v3 imports only the neutral core; the v3 scripts read it from `dist/`, and the replay harness layers imports, scopes, receipts, recovery force and non-service counts over it. | The journal serves one genesis segment of one backing on the local reference venue (M2a); `ErgoVenue` publishing and the reader-entry guard are M2b; recovery kinds in admission, publication force, imports and scopes stay in the harness until slices 3 and 7; no adopted configuration. Not exported from the root barrel. |
