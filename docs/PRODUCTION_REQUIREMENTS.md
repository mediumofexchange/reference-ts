...
| Usable payment | A wallet completes issue → pay → receive → fulfill → redeem, including interruption and exact retry; replaying payment evidence cannot fulfill another invoice | The [local pool wallet fixture](POOL_WALLET.md) connects durable requests, real proofs, verified once-only local fulfillment and retained change across wallet processes. Private HTTPS delivery has durable scoped capabilities, an immutable inbox and lost-acknowledgment recovery, separate from checkpoint verification. Public audit and eleven abrupt wallet commit-boundary cases exist. Encrypted offline exports freeze the source and restore complete state against an independently retained digest; current recovery records, one active copy and protected device storage remain preconditions. Private TLS keys and durable digest-authenticated invitations support restart, fenced rotation and stale-binding rejection. Independent digest authentication and the venue remain locally modeled; qualified device custody, a user pairing channel, deployed endpoint operation and actual redemption remain open. |
| Durable operation | Abrupt termination, lost responses, concurrent writers, disk faults, restored backups and obsolete instances cannot cause conflicting exposed signatures or silently lose accepted operations | PoolStore journals openings, admissions, receipts and publication outbox, with restart fencing and crash tests. The local [pool service](POOL_SERVICE.md) demonstrates separate-process exact retries after a lost commit reply, old-process fencing and graceful restart with a fixture venue. Coordinated backup rollback, custody procedures and external publication remain open. |
| Available, recoverable state | With the original operator offline, an independent reader retrieves and verifies the promised evidence and executes each supported remedy; selective withholding fails explicitly | The [conditional single-backing experiment](POOL_DEPLOYMENT_PROBES.md#conditional-initial-segment-replay) verifies recovery publication force, exact adoption, receipts, non-service counts and repeated same-index returns with real proofs. Independent network retrieval, authenticated venue evidence and a supported pool recovery product remain unbuilt. The frozen path is a case library only. |
