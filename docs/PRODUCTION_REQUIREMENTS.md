...
| Usable payment | A wallet completes issue → pay → receive → fulfill → redeem, including interruption and exact retry; replaying payment evidence cannot fulfill another invoice | The [local pool wallet fixture](POOL_WALLET.md) adds durable requests, payment retries and verified once-only local fulfillment across wallet processes. Proofs and venue are fixtures; real-proof wallet integration, supported custody, private delivery and actual redemption remain open. The transparent pilot remains frozen integration evidence. |
