...
|---|---|---|
| Defined profile | Construction pins the pool's statement layouts, hash functions, proof system and what **E** declares; an explicit visibility/collusion model, witness assumptions and supported failure remedies; no unresolved critical protocol choice | v2 layouts, circuits and keys are pinned. The recovery contract has an unresolved [return safety conflict](POOL_RECOVERY_RETURN_DECISION.md). Return and receipt rules, invalid-checkpoint recovery, v3 layouts, note delivery and deployment choices remain open. |
| Adversarial model | An executable model of §C2, §C2b and §C3 over the pool representation, with two operators, two backings, delayed and dropped publications, replacement, restart and incomplete views; safety and conditional progress checked separately; counterexamples kept as regression vectors | Sequencing, authority and recovery models exist. The [fault candidate](POOL_FAULT_READERS.md) recomputes validation from each reader's evidence; its policy remains unselected. Permanent counterexamples reproduce both invalid-checkpoint progress failure and an old segment spending a venue-settled note after a non-carrying clock reset. Passing these tests demonstrates the failures; no return repair is implemented. |
