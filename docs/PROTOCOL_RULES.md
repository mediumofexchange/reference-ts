...
| A segment imports one finalized prefix per distinct opening commitment, each checked against itself and against the opening's directory root and operator; events deduplicate by (segment, position) with identical effects; a nullifier or output in two distinct events, a total out of bounds, an unnamed prefix or an opening without one is refused. The forest is the prefixes' roots plus the fresh local tree's; one spent set and one output set cover the closure and the tail; totals fold over the deduplicated closure. Witnessed-index ranks, canonical predecessors and finality are the record's. | C2.10.5–7, pool-v2 §10 | `pool/segment.ts` `mergeImports`, constructor, `prefix`; `model/pool-authority.ts` `World.open`, `import`, `merge` | `pool-segment`, `scripts/pool/admission.mjs`, `model/pool-authority.test.ts` | core; witnessed ranks model only |
| Before elective scope changes, witness all live receipted events and the latest signed checkpoint. Restarts preserve the journal; retired instances cannot serve. | C2.10.8–9, C2.8 | `model/pool-authority.ts` `Service`, `World.open` | `model/pool-authority.test.ts` | core rule; model only |
...
11. **Open a segment only from the record's openings** (C2.7, C2.10.4): the header names each backing's canonical predecessor and its current link as the witnessed record fixes them; `Segment` checks the evidence it is handed, not whether it was the right evidence to hand.
