...
uniform-timeout rule also has two costs worth knowing: it is per backing, so a
bundle's halves at operators that cannot see each other can still hold
mismatched timeouts and half-settle (now a party rule in CLAUDE.md, since no
operator can check it), and a stranger who guesses an attempt id and locks first
fixes the timeout for everyone else on that backing — bounded by the standing "a
fresh id per attempt costs nothing", now sharpened to prefer an unpredictable
one.
