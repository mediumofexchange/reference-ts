...
**Queued:** this slice has had no review round. It touches the law, an encoding
and a wire format, which is the profile that has produced a defect in every
round so far — and one open question deserves an angle of its own: whether the
`retired` set is now partly subsumed, since same terms mean the same id and a
withheld object cannot reach a differently-termed lock.
