...
not more than one operator, and the party rule in CLAUDE.md now says so. So the
rule closes the one-backing deadlock — which is real, and worth keeping while it
is the only guard — but the case §C1 is actually about is still open, and only
an attempt id that carries its own terms reaches it.
...
- **Leave `settle`'s receipt documented, with a better reason than "an API
  question".** In the spec's grammar one attempt has exactly one settling
  object — "**the** fully signed exchange object", "all sign", and every lock in
  an honest exchange naming the whole party set (as `c1-swap`'s ring of three
  does) — so any object satisfying one lock satisfies all, and the idempotent
  answer is the right answer for every participant. Two objects under one
  attempt means locks with *different* party sets under one id, which is an id
  collision rather than §C1's exchange. Adding a door argument to serve a shape
  the spec does not produce would be a mechanism derived from a residual.
...
encoder); no salt (predictable ids collide across attempts sharing a party set,
venue and timeout); and generalising the sibling scan instead (zero encoding
cost, but it *worsens* the squat — a stranger would fix your party set too — and
retires neither party rule, being still per backing).
