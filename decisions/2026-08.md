...
commitment defaulted to index zero and opened its gap the moment it was seated —
every door refusing it for a silence it had no chance to break. `publishedInGap`
now measures from `max(last commitment, the index this operator took the role)`,
read through a new `linkIn` beside `operatorIn`. The genesis link's `from` is
zero, so this is the same rule generalised rather than a case beside it. The
simplicity panelist predicted exactly this and called it the one place the
change is not purely subtractive; that is still the honest description.
