...
and three documented residuals: `settle`'s capturable receipt (one attempt has
one party set, so one settling object), the one-holder-two-records collision (a
demand's hash is not `H(attempt terms)`), and the whole squat family at the
level of *representability* rather than refusal. A venue-naming lock simply
cannot name a demand's hash.
...
not build two objects. And `submit`'s lock gate reads the venue before the law,
so an unlawful lock's refusal is labelled by the reader's sync state rather than
by the boundary that refused — the mirror of the ordering slice 28 fixed inside
`unservedRequests`.
