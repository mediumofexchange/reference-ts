...
- **Keep the uniform-timeout rule: the spec already requires it.** §C1's swap
  ...
  An all-or-nothing `committedInTime` was rejected as building around the spec:
  it permits a state §C1 forbids and then manages the consequence, and it buys
  liveness only on the one-backing case that was already safe.
...
edit. It retires the uniform-timeout refusal, both new party rules and both
residuals at once: a stranger's differing timeout is a different id rather than a
squat on yours; an operator that cannot see another's book can still hash, so
the cross-operator half-settle becomes checkable code rather than a rule nobody
enforces; one id then fixes one party set, so one attempt has one object and the
`settle` residual is unreachable; and a demand's hash is not `H(attempt terms)`,
so the self-inflicted collision above goes too. Costs: one more canonical
encoding to audit, a wire bump on the lock message, and the attempt id stops
being 32 free bytes a wallet picks.
