...
## 2026-08-31 — Slice 35d: the book built, and what building it decided

**Question:** Panel B's possession-with-provenance design (R1–R4, its two
riders, and the three probes as failing tests), approved for building by the
handoff. What the panel recommended is in its own entry; this one records what
building it decided that the panel had not.

**R2's pin, generalized — and the spec sentence corrected.** §C2's merged
sentence pinned the takeover object to "the PREDECESSOR's last commitment
witnessed strictly before the effective index", and the dark-heir probe has a
predecessor with no commitment at all — under the literal sentence the second
heir is still bricked. §C2b's snapshot rule already answers this shape ("a
chain whose middle operator never committed reaches past it to the last that
did"), so the object is the BACKING's last commitment strictly before the
seat's effective index — `lastCommitmentInForce(chain, venue, effective − 1n)`,
the same subject rule 35c gave every other reader, identical to the spec
sentence in every ordinary case. One clause added to §C2 on the open spec
branch; merge was pending at the time.

**The seat is the walk's last link, pending or arrived** (`successionAhead`,
one walk answering both the force question and the possession question), and
the walk is memoised in the sequencer against (witnessed index, record count) —
both halves of the key, because a record can land without the clock moving.
The cached links are never handed out, which is the lesson `linkIn`'s deletion
recorded. `serves` is one comparison of the seat against the chain tip's link;
seat values are only ever written for links naming this operator, so the
comparison carries force too.

**In force is not custody, at every co-signing door.** The `inForce` door
check asks `serves` as well, so an heir that skipped the takeover is refused
and told the honest path — it used to pass every door and co-sign onto an
empty log (the round's zero-lead-time probe). `caughtUp` and `adopt` gate on
`serves` for the same reason (adoption is co-signing), which closes the
isInForce-vs-serves divergence 35c recorded as riding along. The door order
swapped so the custody refusal is named before `shut` names the commit: an
in-force heir without the book told to "commit first" would be told to make
the one move that closes the gap and kills the holder's redemption.

**The fast-forward's tail disposition**, which the panel's interleaving rider
demanded probed before the door opened (`scratch/book35d-interleavings.mjs`,
four probes, all pinned as tests):

- A takeover to a NEW seat drops the tail past the committed mark first,
  receipts included, exactly as a return from silence drops it — the tail was
  co-signed in an era that closed (a re-appointment always has an intermediate
  link, since naming the incumbent is refused), and 28b already ruled those
  receipts lapsed. The dropped request is resubmittable as a fresh act; the
  probe shows the fresh receipt at the fresh position.
- A re-sync of the seat already held touches nothing: the tail then is the
  live term's own. A book already past a walk-back target is a no-op, not a
  refusal — the dark-heir-then-reappointment case resumes through it.
- The takeover MARKS what it took on, and the slow-return probe shows why
  that is load-bearing: a rescuer seated into a long silence that dawdles
  before committing meets its own restore, which cuts to the mark — unmarked,
  the restore would empty the taken-on book and the return would root nothing.
- The nonce-gap compound (dead tail + dark heir + open gap): a gap leg the
  holder signed at a nonce that rode the dead tail is skipped by the
  operator's adoption AND by the verifier's fold — both read the same
  restored book, so no reader disagrees — and adopts by itself once the dead
  request refills its nonce, since adoption is re-asked at every door and the
  gap's legs stay the gap's.

**The stale seat's price, stated.** A re-appointed operator's old seat is
stale on its face and roots nothing — the stale book is never asserted, which
was the manufactured shrink fault. What an operator that commits PAST a stale
seat roots instead is a state that drops a backing it is in force for, and
that is the ordinary drop-while-in-force fault, deliberately not excused: the
fast-forward stands open at every index, so the honest order is re-sync, then
commit. The residue this leaves is the panel's kept objection, unchanged: a
predecessor that withholds its committed state traps an honest heir between
the silence grade and the drop fault; and a poisoned pin — the heir's only
commitment a law-valid rewrite — is refused rather than carried (taking it
would make the resumed operator's next commitment prove a rewrite against
itself; the heir's fault stays the heir's).

**Mutation-tested at build time**, per the 34b/35c lesson: seven mutations,
each killed by a named test — serves→isInForce (3 tests), the tail drop
deleted (2), keep ignoring the mark (1), the pin unpinned to latest-now (4),
the takeover's mark deleted (3), the prefix identity check deleted (1 — it
survived the first sweep, and the poisoned-pin test was added to kill it),
the walk cache never invalidating (11).

**Review rounds run:** the implementation round is owed and runs next; its
findings get their own entry.

**Spec change:** the walk-back clause on `spec/the-record-is-the-backings`
(commit "C2: the handover object is the backing's last commitment, as the
snapshot is"); merge was pending at the time.
...
non-service subject. PR was pending at the time.
...
**Decision (2026-08-29):** its own slice, later — real, but a
...
rebuild; the build moved to a fresh instance before the panel
...
- **The first version of the conscription test passed against unmodified code.**
...
hostile operator roots whatever it likes — retiring a verifier's check because the
...
**Decision, 2026-08-29.** Review considered simplicity, security, practicality
and consolidation. The reviewers disagreed on the mechanism, as recorded below.
...
security, practicality, consolidation), all using the same model. The design panel is new
...
and the proposal was accepted for implementation. An attempt id was 32 bytes a wallet
...
**The review round, and the four defects it found.** Five review angles over the
...
4. **Heterogeneous timeouts under one attempt deadlocked.** Locks dying at
   ...
   withdraw either. **Decision (2026-08-28):** one timeout per
...
**Both were reviewed from four angles —
...
yet Ergo is half-built and Bitcoin is a target — as a
...
**Direction (2026-08-27):** Ergo first and already in progress;
...
is the first priority. **The hard rule is §10's: do not wrap
...
**Decision (2026-08-27):** split, index, and cut — adding without removing
balloons everything.
...
**Decision (2026-08-25):** rename all of them to the `moe/` namespace and
...
**Question:** the last item in the queue from the 2026-08-22 audit, and slice
...
**The review round** (three review angles: the count adversarially; the
...
**Spec change:** *[Both made 2026-08-25:
...
**Question:** the audit's third question concerned P paying in claims:
...
**The review round** (one review angle: attack the reservation read). Three
...
as recorded here.]* As recorded: §C3's presentation section reads
...
lapse, not the backer's failure."*

## 2026-08-25 - Slice 29: a dead successor does not end the chain

**Question:** the audit's second question concerned whether the chain walk passes
...
witnessed). This is evaluated per backing, like the chain. `successionOf` took the earliest
...
**The review round** (two review angles: the walk adversarially; tests as claims
and docs). Besides the instability and
...
money-from-first-principles@589316a, as recorded here]***.** §C2's replacement paragraph
...
re-naming the incumbent revokes a successor not yet in force."* The amendment is recorded above.

## 2026-08-24 - Slice 28b: the receipt names its era, and the era is the backing's own

**Question:** the second half of audit item 1, split off in 28a and queued:
...
**Review scope and limitation.** An outage prevented the three planned review
passes from completing. They were rerun using the implementation session's model,
a departure from the then-required review-model selection. This did not establish
model diversity. The findings and resulting changes follow:
...
**Spec change:** *[Both candidates made 2026-08-25:
...
**Question:** the audit's first open item, decided on 2026-08-22: an
...
gap conflict). **Split into two implementation steps**: 28a is the
...
**Decision:** The plan was approved as proposed; the review round reshaped two
...
**The review round (three review angles: the law and the book; the doors and the
...
review angles over the fix round's diff; sixteen mutations run against the suite).
...
**OPEN, pinned for 28b — the second half of the recorded item 1.** After a restore a
...
**Rethink, recorded for decision:** the doors each hand-write one order (shape →
...
**Spec change:** one sentence initially proposed for decision rather than amended. *[Made 2026-08-24 in money-from-first-principles@a5665d1,
with the three other pending sentences (@e370e59 slice 27's
...
<a id="2026-08-22---the-audit-six-angles-over-the-merged-code-what-was-wrong-and-what-is-the-maintainers-to-decide"></a>
## 2026-08-22 - Implementation audit findings and protocol questions

**Question:** an audit examined the merged implementation against the
paper — six review angles (sets and exits; the gap path and succession; bundles,
...
questions are below, open, for decision.
...
- *An attempt id names one attempt on one backing, for the locks a commit can
  ...
  bind the exchange's backing set instead is the question below.
...
**The review round (four review angles: the law and encodings; the sequencer and
...
**Open, for decision — each proven by a run, none decided here.**
...
1. **The uncommitted tail on return from silence** (audit-B-2, -4). An operator
   ...
   afterwards. The reading is: a receipt proves an act the operator took, and after
   ...
   against it. A law-of-the-tail change remained to be decided.
...
**Decided (2026-08-22 evening):** Accept the recommendations subject to
preserving the core design, simplicity and separation of backings. The initial
assessment identified no apparent conflict; the changes still required implementation
and verification under those constraints:
...
4. **Cross-operator presentation:** out of scope, an extension; recorded, not
   ...
   money-from-first-principles@b62f6ee.]*
...
**Decision:** The plan was approved as proposed:
...
money-from-first-principles@e370e59.]* §C3 says "a demand
...
creation is — the same index is not a window."* The amendment is recorded above.
...
**Decision:** Adopt the proposal with simplifications where practical:
...
- **The commit door.** An earlier edit closed the release path for a paying lock settled
  alone while leaving the commit path open: the paying lock is a one-party lock with
...
**Decision:** Adopt the recommendation; the alternative fork was recorded and
not taken:
...
**Decision: the two exits are complements on the timeout**, as they are on
...
**It reverses a slice-22 behaviour approved earlier**, and was recorded as
...
reliance legs" and the review questions turned it into the general case, which is
...
- **Atomicity scope:** Redemption with reliances and a multi-backing transfer
  both involve a bundle of claims. The reliance set is the bundle R names,
  so the implementation uses a general mechanism for both.

- **Sequencer scope:** One proposal left verification of the reliance transfers
  to the backer's wallet, which could accept or refuse redemption. Slice 23
  already provided that check. It did not protect the **holder's** side: invariant 8 makes a transfer irreversible, so
...
- **Shared settlement:** Another proposal locked claims independently and
  triggered settlement when all sequencers signed. Locks already provided the
  escrow mechanism, but signatures alone did not provide a shared trigger: a
...
- **Witnessing:** Waiting for each sequencer's witnessed commitment was proposed
  as a way to avoid a separate publication. But a commitment is a **root**, so "O2 witnessed its
...
- **Scope of atomicity:** It is needed where parties lack trust; ordinary local
  purchases can use independent claims. The
...
- **One commit object, not one per sequencer.** With n objects, each sequencer
  would need to check the whole set. One object lets it match its own lock
  without reading other backings. This replaces the earlier recommendation
  of n per-leg releases plus a whole-set check.
...
**Decision:** The review selected one option and reversed an earlier plan.
...
- **A leg is reserved by a `lock` in the LEG's own log**, signed by the holder
  ...
  R(b) and q determine the legs. Put for decision as a fork rather than taken silently:
  ...
  the selected option was **A**. B's saving is signatures; its cost is the thing `provesHolding`,
...
**Spec change:** *[Made 2026-08-24 in money-from-first-principles@213d9e1.]*
§8b's example could gain the discriminating case -
...
so the paper is not wrong, only silent. An explicit example remained a possible documentation improvement.
...
- **One helper, `answering` in venue.ts**, running a verifier's body,
  ...
  something an implementer has to remember at each site.
...
**The review challenge, and where it landed.** The review questioned pro-rata:
whether backers should choose allocation in a shortfall and whether a large theft
would make each claim nearly worthless. The paper provides the governing analysis:

- **Allocation belongs to the backer's terms**, and the earlier claim that
  pro-rata was fairer was a norm the paper does not hold. §5: "what the core defers is a
...
- **Restricting payouts to trusted recipients is forbidden in the terms** by invariant 19:
...
- **The earlier framing was wrong twice over:** pro-rata is not a workout that fires on a
...
- **Prevention:** prevention beats remedy, through
...
outstanding count a payout reads after a revocation" — a one-sentence clarification. The code already read the standing count, so
...
- **`provesHolding` and `isSilent` are deliberately unchanged**, and this reverses
  the previously accepted recommendation. Relaxing `provesHolding`'s "latest" to "latest
...
**Spec change:** two, both made directly and minimally as recorded here.
...
**Question:** the layout was blocked on questions previously intended for the
Basis authors. The selected approach was to develop independently, retain useful
ideas and resolve differences against the node's own source
...
reads the *current* tracker box; four of the nine venue reads ask about a past
index, so the implementation needs a historical read.
...
- **None of Ergo.** Modelling boxes and registers without a node means inventing
  ...
  client. What a real venue actually forces is something the implementation had never built.
...
**The convergence is not coincidence, and is not only lineage.** The paper
...
- **The reserve is not a foreign concept to import. It is our own deferred
  chain-asset leg.** The reasoning is: a **hard** reserve, one
...
- **Hash-linked, agreed before building.** Each replacement names its
...
**Decision:** the encoding first, so the replacement rule lands as one
...
a boolean where the situation has more than two shapes. Separately, the
question of an operator restoring from continuously saved state exposed an
...
**Question:** review of the merged implementation looked for
...
**Decision: the window is not patched, and nothing is added to reach
...
- **Illiquidity is the rule, and it is the design rather than a workaround.**
  ...
  was explicit: liquidity resumes when the operator returns
...
- **The repairs that would reach further do not survive blinding.** The one that
  ...
  last commitment never saw. The proposal was rejected because this slice is the
...
honest payee finds the nonce spent. The review considered whether the payee can at least
come away with **proof of fraud**, since the system needs trust in the operator
regardless.

**Decision: yes, and the proof needs nothing from the operator.**
...
until the review noted that claims travel - Alice pays Bob, Bob pays Carol, and
...
**Operator constructions, recorded in CLAUDE.md rather than built:** threshold
and replicated operator options were recorded for later evaluation.
...
- And reviewing *that*: a claimant who pre-empts a genuine request by publishing
  ...
  always first. **The earlier analysis closed the wrong half of this.** A
  request that pays the claimant is no evidence of a spend and no longer folds -
  which is true, and buys nothing: a keypair is free, so the claimant
...
**Question:** the review considered which recorded decisions no longer serve the goal -
...
**Question:** the review examined the merged implementation against the
...
**Decision:** Each choice responds to a demonstrated attack rather than an
argument:
...
- **Correction to the earlier scoping.** The initial analysis treated this as
  "lifecycle only" because
...
**Decision:** state the rule once and enforce it in one place.
...
**Decision:** `makeBacking` freezes the backing object, its `reliance`
...
**Decision:** two independent guards. (1) `makeBacking` rejects an
...
**Decision:** keep the domain separation — signing a bare 32-byte hash
