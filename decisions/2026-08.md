...
- **The salt had three defaults, and one fired where the field is required.**
  ...
  has `salt` as a required field — so an entry missing it got well-formed
  canonical bytes instead of `key32`'s refusal, two distinct entries shared one
  identity, and `applyEntry` then threw a bare `TypeError` where the contract is
  a named boundary. The default is gone; optionality now lives once, at the
  `LockOp` boundary that declares the field optional.
- **An unsalted venue-naming lock was accepted**, and it is the shape the
  ...
  the mirror of the set leg's rule beside it. The law cannot check that a salt is
  random; it can check that the one value reachable by omission was not used, and
  that is a different and exact property (`scratch/q3-unsalted-path.mjs`).
