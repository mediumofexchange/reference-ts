...
4. **The one-per-witnessed-index guard is deleted, not layered.** Its
   purpose is dissolved twice over — the era is no longer an index, and with
   one commitment in flight a second is signed only after the first is in
   the record, so two can never land at one index.
...
7. **The floor rises to `2·lag + 1`.** Slice 38 sized the floor for a
   ...
   lag+1, so the rule-holder can put the whole safe range inside the
   operator's in-flight window. Measured over every alignment and every
   candidate floor (`scratch/panel39/security/probes/s4b-floor-full-sweep.mjs`):
   at lag 4 a floor of `lag+4` still leaves an alignment with no safe clock,
   and only `2·lag+1` survives all of them. **The security angle's F7 said
   one extra index sufficed; its sweep tried three alignments and the worst
   is later than the three.** At lag zero `2·lag+1` is `lag+1`, so no
   `LocalVenue` fixture moves.
