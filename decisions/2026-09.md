...
- **The opening claim was not bound to the record.** Its four conditions —
  ...
  configuration it was in. **Fixed: an opening costs one exhibit**, the
  record's last commitment for the backing shown to carry nothing for it,
  through the walk's own step check (`recorded`), asked once. What the door
  still cannot tell is exactly the indistinguishable case, and the signature
  answers for that. `opening` is now a list of `{ backing, record }`; the
...
1. **`takeOver` is one walk.** The book a seat takes on is the one the
   ...
   with every existing call [as built: for TypeScript callers; an explicit
   `undefined` third argument, meaningful before, is refused by name now — the
   regression round].
