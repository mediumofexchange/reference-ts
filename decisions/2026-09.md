...
**Status: the regression round the walk fix owed, its fixes built, and ONE
review of those fixes still owed before `slice/36-the-resume` merges.** Three
...
- **The opening claim was not bound to the record.** Its four conditions —
  ...
  away, never one; three door-side bounds were probed and each bricks the
  honest new backing or makes `opening` redundant (the review of the fixes,
  below). Where the record holds no in-force commitment at all there is
  nothing to exhibit and nothing to claim — refused, naming
  `takeOver(backing)`, which takes the empty book (the F2 heir). `opening` is now a list of `{ backing, record }`; the
...
- **`opening` where the walk is free is not refused** and cannot be: an heir
  whose predecessor's last carries a full book may sign an empty one instead
  of taking it — no power an operator's key lacked, provable, and no refusal
  for CLAUDE.md's rule to attach to. A warning-shaped reader is a design
...
**Next, and owed:** the review of THIS fix batch — CLAUDE.md's rule, and every
round so far has found a real bug in the fixes — then the merge and any push,
the maintainer's call, with the spec branch `spec/the-walk` (two commits,
unpushed) to merge upstream first, and three things flagged for the
