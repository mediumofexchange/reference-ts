...
try {
  ...
  await test("withdrawal without a standing demand and unauthenticated imports produce no partial audit", async () => {
    ...
    assert.equal(recovery.status, "invalid-local-replay"); assert.equal(recovery.check, "DEMAND");
    assert.equal(recovery.audit, null); assert.deepEqual(recovery.candidates, []);
  ...
  const scopeReceiptPairs = [
    [scoped.receipts.payload, scoped.receipts.result], [scoped.receipts.joinedPayload, scoped.receipts.joinedResult],
    [scopeRecovery.receipts.payload, scopeRecovery.receipts.result], [scopeRecovery.receipts.adoptedPayload, scopeRecovery.receipts.adoptedResult],
  ];
  ...
  const intrinsicPairs = [...imported.intrinsicCases, ...silent.intrinsicCases];
  await test("compact silence clocks, predecessor refusal and repair agree in portable packages", async () => {
    for (const item of intrinsicPairs) assert.deepEqual(await replayEvidencePackage(portable(item.payload), verifier, codec), item.result);
  });
  const lapsePairs = [scoped.lapse, scopeRecovery.lapse, silent.lapse];
  ...
  await test("compact adopted-block refusals agree in portable packages", async () => {
    for (const item of recovery.intrinsicCases) assert.deepEqual(await replayEvidencePackage(portable(item.payload), verifier, codec), item.result);
  });
  ...
  await test("fresh public verifier has no wallet seed; separate receiver restores from public evidence only", () => {
    ...
    for (const item of intrinsicPairs) assert.deepEqual(worker(item.payload), item.result);
    ...
    for (const { payload, result } of lapsePairs) assert.deepEqual(worker(payload), result);
  ...
  checkCandidateSources(manifest);
  const report = { schema: "moe-v3-local-replay-experiment-21", specification: "183c09f", node: process.version,
    ...
    scopeReceipts: { packageBytes: portable(scopeRecovery.receipts.adoptedPayload).package.length,
