...
try {
  ...
  if (withErgo) report.limits.push("The candidate Ergo adapter checks exact transaction decoding, contained per transaction under the reader's fuel and memory budget, and roots against independently selected synthetic headers. No proof of work, chain selection, decoder node equivalence for hostile inputs, node acceptance or venue-profile adoption is established.");
