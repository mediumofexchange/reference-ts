...
async function replayTrail({ selection, terms, scopedTerms, header, verifier, codec, contextReceipt }, snapshot, trail,
  ...
  const events = new Map(from?.events), totals = new Map(
  ...
  for (const bytes of base === undefined ? trail.records : trail.records.slice(Number(base.position))) {
...
function resumeKeyOf({ selection, terms, scopedTerms, verifier, contextReceipt }, snapshot, { imported, block, openingIndex }) {
...
const sameResumeKey = (a, b) => a.text === b.text && a.verifier === b.verifier && a.contextReceipt === b.contextReceipt &&
  a.imported === b.imported && (a.block === b.block || (a.block.length === 0 && b.block.length === 0));
...
function resumable(codec, segment, trail, lastValid, key) {
...
async function classifyCarrying(context, ranges, evidence) {
  ...
  for (const c of ranges.carrying) {
    ...
    if (c.position !== "selected") {
      ...
      // Terms are resolved by name from any strictly verifying field (§12.1).
      if (resolveTerms(codec, trails, s.segment, { backing: selection.backing }, 0) === undefined) throw new EvidenceRefusal("unresolved-evidence");
    }
    let verdict;
    try {
...
export async function replayLocalPackage(input, verifier, codec) {
  ...
  try {
    ...
    // pool-v3 §12.1: a failing field is ignored when another supplied field of the segment verifies.
    const suppliedTrails = [trail, ...decodedTrails(byteList(supplied.trails, "trails"), codec)];
    const signed = resolveTerms(codec, suppliedTrails, sha256(trail.header), header.entries[selectedEntry], selectedEntry) ?? trail.terms[selectedEntry];
    const signedTerms = header.entries.map((entry, i) => resolveTerms(codec, suppliedTrails, sha256(trail.header), entry, i) ?? trail.terms[i]);
...
export async function replayEvidencePackage(input, verifier, codec) {
  try {
    ...
    const trailBytes = [payloads(6).find(payload => same(payload, codec.encodeTrail(served, LIMITS))) ?? codec.encodeTrail(served, LIMITS)];
