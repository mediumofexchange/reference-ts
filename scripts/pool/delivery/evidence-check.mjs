...
import { evidenceCodecs as codec, inspectRestorationEvidence, LIMITS } from "./evidence-reader.mjs";
import { sourceClosure, sourceHashes } from "../v3/provenance.mjs";
...
function worker(payload) {
  const child = spawnSync(process.execPath, [join(here, "evidence-worker.mjs")], {
    input: serialize(payload), cwd: scratch, timeout: 30_000, maxBuffer: 1_048_576, windowsHide: true,
  });
...
test("request journal cannot override public spentness or supply an output", () => {
  for (const journal of [[], [{ request: "current", fulfilled: false }], [{ request: "stale", value: "7" }]]) {
    ...
    const child = spawnSync(process.execPath, [join(here, "evidence-worker.mjs")], {
      input: serialize(payload), cwd: scratch, timeout: 30_000, windowsHide: true,
    });
...
// The relative import closure of the check and its worker, dist modules with their src sources.
const sources = sourceClosure(["scripts/pool/delivery/evidence-check.mjs", "scripts/pool/delivery/evidence-worker.mjs"]);
