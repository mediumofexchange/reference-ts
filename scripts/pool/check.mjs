...
try {
  ...
  for (const [label, mutate] of [
    ...
    ['issue domain limb overflow', v => { v.domain[0] = (1n << 128n).toString(); }],
    ['issue segment limb overflow', v => { v.segment[1] = (1n << 128n).toString(); }],
    ['issue backing limb overflow', v => { v.backing[1] = (1n << 128n).toString(); }],
    ['issue link limb overflow', v => { v.link[0] = (1n << 128n).toString(); }],
    ['issue backing outside the scope', async v => { v.backing = f.foreign; await refreshIssue(v); }],
    ['issue wrong scope root', v => { v.scope = field(1); }],
    ['issue wrong link', v => { v.link = f.linkB; }],
    ['issue wrong scope sibling', v => { v.scope_siblings[0] = field(1); }],
    ['issue wrong scope direction', v => { v.scope_right[0] = !v.scope_right[0]; }],
    ['issue scope path of the other entry', v => { const p = f.scope.path(1n); v.scope_siblings = p.siblings; v.scope_right = p.right; }],
  ...
  for (const i of [0, 1]) {
    for (const [label, mutate] of [
      ...
      ['input wrong link', v => { v.links[i] = f.linkB; }],
      ['input wrong scope sibling', v => { v.scope_siblings[i][15] = field(1); }],
      ['input wrong scope direction', v => { v.scope_right[i][0] = !v.scope_right[i][0]; }],
      ['input link limb overflow', v => { v.links[i][1] = (1n << 128n).toString(); }],
  ...
  for (const [label, mutate] of [
    ...
    ['wrong anchor', v => { v.anchors[0] = field(1); }],
    ...
    ['wrong scope root', v => { v.scope = field(1); }],
    ...
    ['changed domain with recomputed nullifiers and outputs', async v => { v.domain[0] = '18'; await f.refresh(v); }],
  ...
  await rejects('second input under the first anchor when the trees differ', 'spend', f.twoAnchors, v => { v.anchors[1] = v.anchors[0]; });
  ...
  for (const [label, mutate] of [
    ...
    ['burn wrong link', v => { v.link = f.linkB; }],
    ['burn wrong scope root', v => { v.scope = field(1); }],
    ...
    ['burn wrong first anchor', v => { v.anchors[0] = field(1); }],
