...
import { DEFAULT_ERGO_DEPTH, ErgoVenue, ergoAnchorContext, ergoProfile } from "../../dist/ergo.js";
import { ergoProfileIdentity } from "../../dist/ergo-profile.js";
...
// The sources are hashed now, before any work, so the report names the files that produced it: this script and the
// runtime it reads from dist/ (run `npm run build` first), each with its src/ source.
const files = sourceHashes(sourceClosure(["experiments/ergo-range/runtime-sync.mjs", "package-lock.json", "tsconfig.json", "tsconfig.build.json"]));
...
report = {
  ...
  limitations: [
    ...
    "The locations are throwaway keys, so every answer is empty: attribution of real records on a node is the publisher check's, on the testnet through the same framer.",
