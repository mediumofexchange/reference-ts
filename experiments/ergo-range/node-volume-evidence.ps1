...
function Assert-VolumeNodeProcess($Process) {
    if ($null -eq $Process -or -not $Process.LimitsReadBackBeforeResume -or -not $Process.JobEmptyAfterCleanup -or
        ...
        $Process.ElapsedMs -le 0 -or $Process.ElapsedMs -gt 120000 -or
...
function Assert-VolumeNodeTraffic($State, $Budget) {
    if (-not $State.finalAttempted -or $State.readError -or -not $Budget.AccountingValid -or $Budget.StopReason -or
