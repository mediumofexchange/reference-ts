...
if ($observed.Count -ne 3 -or $observed.status -cne 'passed' -or $observed.cases -ne 16 -or $observed.test -cne 'status-and-write-roots') { throw 'Unexpected pure Java unit evidence' }
$classPins=[ordered]@{}
...
[ordered]@{
    ...
    limitations=@('Compilation and 16 pure Java status/path checks only; worker main and JNI/database work are not executed.',
