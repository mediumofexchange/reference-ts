...
// Usage, from the repository root on Node 24 after `npm ci` and `npm run build` (it reads the runtime from dist/):
...
// runtime it reads from dist/, each with its src/ source.
const files = sourceHashes(sourceClosure(["experiments/ergo-range/header-verify.mjs", "package-lock.json", "tsconfig.json", "tsconfig.build.json"]));
