...
- **Exact directory descent** (`src/pool/descent.ts`): selects a predecessor candidate relative to a held child, using authenticated directory absence and snapshot-bound whole-scope lapse. Live invalid history and unavailable evidence never permit fallback. This reader does not certify replay, canonical imports or whole-scope finality; those remain the next sequencing step.
- **The private authority model** (`model/pool-authority.ts`): C1.2/C2.10 private scope authorization, whole-scope finality and canonical finalized-history imports, with counterexamples. It uses ideal cryptography and local journal ownership; `src/pool/` now holds the layouts and circuits, and production persistence, finality and receipt classification remain the sequencer's.
