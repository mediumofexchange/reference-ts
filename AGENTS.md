...
- **The invalid-checkpoint candidate** (`model/pool-fault.ts`): candidate A of `docs/POOL_FAULT_RECOVERY_PROPOSAL.md` over the recovery model — a held checkpoint is valid, excluded on authenticated evidence, or unresolved; the snapshot, count, clock, descent and receipt reader pass an excluded checkpoint and stop at an unresolved one; an excluded checkpoint faults its segment and repair is a new segment; an excluded carrying commitment does not close the interval. Not normative: the contracts still block on invalid live evidence, and the choice is recorded as open. For it the authority model carries `Recorded.reason` and the hooks `excluded`, `blocking` and `admissible`.
