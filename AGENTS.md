...
- **Exact directory descent** (`src/pool/descent.ts`): selects a predecessor candidate relative to a held child, using authenticated directory absence and snapshot-bound whole-scope lapse. Live invalid history and unavailable evidence never permit fallback. This reader does not certify replay, canonical imports or whole-scope finality; those are checked by the checkpoint validator.
- **Whole-scope checkpoint validation** (`src/pool/checkpoint.ts`): validates exact held checkpoints and transitive canonical imports with per-backing descent before replay. Later checkpoints preserve the finalized prefix of their segment. This read returns historical finality, not current spendability or permission to open service. Canonical opening construction and durable sequencing remain next.
