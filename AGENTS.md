...
- **The Ergo read-only venue adapter** (`ergo.ts`): kept as the venue direction, not exported from the root barrel, rewritten when the commitment format is final. Refresh builds a private candidate and publishes the complete snapshot at once; record reads refuse while it runs, and failure retains the last successful snapshot at its old index.
...
- **`src/pool/`:** the claim layer, construction `moe/pool/v2` (pool-v2 §§1–13): field encodings, the host Poseidon2 pinned to the backend's vectors, notes over the construction domain, the note tree, the spent set, the scope tree (`scope.ts`), the configuration/segment/statement/history/snapshot frames (`statement.ts`), and `Segment` (`segment.ts`) — finalized import with deduplication and conflict refusal, admission against one committed view, the directory over the whole scope, the served trail, finalized prefixes, and replay through supplied checkpoint evidence. `pool/receipt.ts` signs §9's receipt bytes under a segment's authority and checks statement identity, exact evidence and replayed history inclusion; durable issuance is in PoolStore, and witnessed finality is a separate record question. `pool/circuits/` holds the pinned issue, spend and burn sources with their bytecode and key identities; `pool/barretenberg.ts` is the only module that imports `@aztec/bb.js` (an optional peer dependency). `npm run check:pool` compiles the circuits, proves them, and drives two segments across an operator replacement with real proofs; `test/pool-*.test.ts` exercise the state machine and import with an oracle verifier. What the claim layer does not decide is the record's: which opening is canonical, whether a checkpoint is final, whether a term ended (C2.10.3–4, C2.7); the read-only authority, descent and checkpoint modules below now check these record facts; durable sequencing is in PoolStore below; recovery remains next. The historical v1 runtime is in git history only. The experiment remains until its crash and journal cases have also moved to the pool path.
- **Record-derived scope authority** (`src/pool/authority.ts`): a read-only snapshot of signed backing terms and the witnessed replacement chain, reusing `successionAhead`. Checks the full scope at a witnessed index and derives deadlines for `pool/schedule.ts`; requires all backings to declare the same venue. Does not authenticate a header, derive canonical openings or establish finality. Refresh the view before acting on a later record.
- **Exact directory descent** (`src/pool/descent.ts`): selects a predecessor candidate relative to a held child or the current record, using authenticated directory absence and snapshot-bound whole-scope lapse. Live invalid history and unavailable evidence never permit fallback. This reader does not certify replay, canonical imports or whole-scope finality; those are checked by the checkpoint validator.
- **Whole-scope checkpoint validation** (`src/pool/checkpoint.ts`): validates exact held checkpoints and transitive canonical imports with per-backing descent before replay. C2b.1 requires newly finalized issuance strictly before revocation; later checkpoints preserve proven earlier prefixes. Same-index revocation changes invalidate the read. This returns historical finality, not current spendability or permission to open service. Silence recovery remains outside pool-v2 §7.4.
- **Canonical opening construction** (`src/pool/opening.ts`): derives current scope and opening checkpoints before a child exists, batch-validates their required histories, and prepares an empty local segment. The next sequence comes from the operator-wide durable signed counter, including failed publications. Preparation reserves nothing, does not discard live tails, and cannot authorize signing or service.
- **Pool receipt record readers** (`src/pool/receipt-record.ts`; `docs/POOL_RECEIPTS.md`): exact signed-sequence resolution, authenticated complete scope terms and inclusion in a supplied source checkpoint. Checkpoint validation returns replayed local admission records and checks prospective revocation. `pool/receipt-repair.ts` classifies C2.10.9a at a supplied proven repair boundary, and `pool/receipt-status.ts` reads C2.10.9b's present verdict — final, contradicted, abandoned, lapsed or pending — by walking the operator's held commitments from `after` to the earliest term end or the first checkpoint of another segment carrying a scope backing, passing over checkpoints that carry none. These readers authorize no discard; silence recovery remains next.
- **Durable pool sequencing** (`src/pool/store.ts`, Node 24 subpath; `docs/POOL_STORE.md`): journals openings, admissions, original receipts, the operator-wide signed counter and publication outbox. Retains complete used canonical evidence, including same-segment predecessors and history-free descent proofs; revalidates imports before new service after restart. Proven pre-revocation issuance remains importable through later checkpoints. Replays local statements, fences earlier handles, waits the restart lag and preserves live tails across elective scope changes. Refuses silence clauses. Recovery and service transport remain next.
- **The private authority model** (`model/pool-authority.ts`): C1.2/C2.10 private scope authorization, whole-scope finality and canonical finalized-history imports, with counterexamples. It uses ideal cryptography and local journal ownership; `src/pool/` now holds the layouts and circuits, and the durable store implements local journal ownership; `classify` is the receipt verdict with its carriage counterexample.
- **The presentation and recovery model** (`model/pool-recovery.ts`): `pool-recovery.md` over the authority model — the demand, lock, acceptance and settlement at the door (C3.7–8), the request and the count (C2b.5.1–2), the no-commitment clock, the snapshot and the adoption index (C2b.6.1, C2b.3.1), publications with force at the venue (C2b.3.2–3) and the return as a new segment with its adopted block and historical silence retirement of continuation and unfinished receipts (C2b.4.1–3), each with a counterexample under a departure, and `recoveryViolations` as the independent semantic check. For it the authority model carries the demand, withdraw, settle and request kinds, the pending-lock set and standing demands in state, `Service.adopt`, and the hooks `adoptable`, `passedByRule`, `replayed`, `serving` and `discardable`. Nothing in `src/` implements these objects; they are `moe/pool/v3`'s, and PoolStore still refuses silence clauses.
- **The invalid-checkpoint candidate** (`model/pool-fault.ts`; `docs/POOL_FAULT_READERS.md`): candidate A of `docs/POOL_FAULT_RECOVERY_PROPOSAL.md` over the recovery model. Witnessing and reader validation are separate; fresh readers replay original record prefixes from their own evidence, with memoization only within a read. Snapshot, count, clock, descent, imports and both receipt readers pass evidenced exclusions and refuse unresolved dependencies. Term lapse needs scope/terms; silence lapse can recursively require unrelated fault evidence even for a non-carrying clock step. `docs/POOL_FAULT_EVIDENCE_REVIEW.md` recommends term-only non-carrying resets with explicit stale-signing suppression costs and demonstrates that v2 lacks the ideal model's exact checkpoint-proof binding. Clock, segment-wide fault and receipt precedence remain research choices. Not normative: the contracts still block on invalid live evidence; adoption needs the exact evidence, liability and complete-range retrieval contracts.
