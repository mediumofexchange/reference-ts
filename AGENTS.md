...
- **Durable pool sequencing** (`src/pool/store.ts`, Node 24 subpath; `docs/POOL_STORE.md`): journals openings, admissions, original receipts, the operator-wide signed counter and publication outbox. Replays retained imports and local statements, fences earlier handles, waits the restart lag and preserves live tails across elective scope changes. Refuses silence clauses and revoked histories requiring the forthcoming recovery reader. Receipt classification, recovery and service transport remain next.
