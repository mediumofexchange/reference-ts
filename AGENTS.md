...
- **`src/pool/`:** the claim layer (pool-v1 §§1–8): field encodings, the host Poseidon2 pinned to the backend's vectors, notes, the note tree, the spent set, the configuration/statement/history/snapshot frames, and `Pool` — admission against one committed view, the served trail, and replay. `pool/circuits/` holds the pinned issue, spend and burn sources with their bytecode and key identities; `pool/barretenberg.ts` is the only module that imports `@aztec/bb.js` (an optional peer dependency). `npm run check:pool` compiles the circuits, proves them, and drives `Pool` with real proofs; `test/pool-*.test.ts` exercise the state machine with an oracle verifier. The receipt and commitment envelopes over the pool's fields are the next step's. The experiment remains until its crash and journal cases have also moved to the pool path.
