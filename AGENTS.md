...
Choose the next slice by its contribution to the smallest supported product, dependencies and the highest consequential uncertainty. Prefer a complete user/recovery path or end-to-end experiment over one preparatory change. Group connected implementation, relevant verification, review, execution and evidence into one sustained slice; continue through intermediate commits and context windows while useful authorized work remains. Stop for a real dependency or authorization boundary, not merely because one test or helper is complete. State the expected observable result and completion checks in `WORK.md` before substantial work. Keep independent commits/reviews where they reduce risk; larger slices do not weaken verification or justify unrelated scope.
...
- Select checks from the changed behavior and its callers/dependencies. Isolated experiment or tooling changes need their relevant syntax, focused tests and acceptance run, not an unrelated full protocol suite. Run `npm run check` for shared runtime/protocol behavior, public APIs, dependencies/toolchains, packaging, broadly affecting build/CI changes, or uncertain impact. Reuse a passing full-suite baseline when those inputs are unchanged; record that baseline, the affected checks and any evidence gap. Report failures without weakening checks. Required CI gates remain in force; do not duplicate an unchanged passing CI run locally by habit. `npm run check:pool-wallet-real` exercises the real-proof wallet flow separately.
...
Final reports state delivered behavior, checks/review, merge/push state and remaining limits. Keep a coarse product-effort estimate in WORK.md using [production requirements](docs/PRODUCTION_REQUIREMENTS.md#progress-estimate). After meaningful product progress or a consequential blocker, reassess briefly from evidence already gathered; do not run extra research or delegate work just to estimate it. Credit reusable progress before release gates close. Report a rounded estimate/range and main blockers when materially changed or requested; omit unchanged percentages from routine fixes and tooling reports. Never infer progress from commit/test counts or increase it to show movement. Workflow work alone does not advance product completion.
