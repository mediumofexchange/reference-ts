...
- **Whole-scope checkpoint validation** (`src/pool/checkpoint.ts`): validates exact held checkpoints and transitive canonical imports with per-backing descent before replay. Later checkpoints preserve the finalized prefix of their segment. This read returns historical finality, not current spendability or permission to open service. Canonical opening construction is below; durable sequencing is in PoolStore below; recovery remains next.
...
- **Pool receipt record readers** (`src/pool/receipt-record.ts`; `docs/POOL_RECEIPTS.md`): exact signed-sequence resolution, authenticated complete scope terms and inclusion in a supplied source checkpoint. Checkpoint validation returns replayed local admission records. These facts do not authorize discard or classify global lapse/fault; revocation and silence recovery remain next.
