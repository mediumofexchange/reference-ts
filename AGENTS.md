...
- **`src/pool/`:** the claim layer (pool-v1 §§1–8): field encodings, the host Poseidon2 pinned to the backend's vectors, notes, the note tree, the spent set, the configuration/statement/history/snapshot frames, and `Pool` — admission against one committed view, the served trail, and replay. `pool/receipt.ts` signs §7's receipt bytes and checks the expected operator/configuration, statement identity, exact evidence and replayed history inclusion; it does not issue receipts durably or establish witnessed finality. `pool/circuits/` holds the pinned issue, spend and burn sources with their bytecode and key identities; `pool/barretenberg.ts` is the only module that imports `@aztec/bb.js` (an optional peer dependency). `npm run check:pool` compiles the circuits, proves them, and drives `Pool` and its receipt envelopes with real proofs; `test/pool-*.test.ts` exercise the state machine with an oracle verifier. This is historical fixed-operator v1 evidence. The approved replacement contract requires new layouts and circuits before the pool sequencer, commitment schedule and durable receipt issuance. The experiment remains until its crash and journal cases have also moved to the pool path.
- **The private authority model** (`model/pool-authority.ts`): C1.2/C2.10 private scope authorization, whole-scope finality and canonical finalized-history imports, with counterexamples. It uses ideal cryptography and local journal ownership; layouts, circuits, production persistence and receipt classification are separate work.
